...
function loadModel2Sence() {
  loadModel.traverse((child) => {
    ...
    if (child.children && child.type.toLocaleLowerCase() === "group") {
      ...
      const diffArr: Array<undefined> = Array.from({ length: diff }).fill(
        undefined,
      ) as Array<undefined>;
...
function createMaterial(
  ...
  drawingContext.fillText(
    text,
    textLen >= 10 ? 50 : textLen >= 8 ? 80 : width / 2 - widthOffset,
  ...
  if (intersects.length > 0) {
    ...
    if (intersectedObject instanceof THREE.Mesh) {
      if (intersectedObject.userData?.projectId) {
        ...
        if (
          !intersectedObject.name.includes("路牌2_反") &&
          !intersectedObject.name.includes("路牌1主轴")
        ) {
          $message.show({
            message: "当前没有项目",
          });
